<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="app-page-title">Tambah Stok</h2>
            </div>
    ...
    <div class="mx-auto max-w-3xl">
        <section class="app-card">
            <form action="{{ route('stocks.store') }}" method="POST" class="grid gap-5 sm:grid-cols-2">
                @csrf

                <div class="sm:col-span-2">
                    <x-input-label for="name" value="Name" />
                    <x-text-input id="name" name="name" type="text" class="mt-2" :value="old('name')" required />
                    @error('name')
                ...
                <div>
                    ...
                    <x-text-input id="kode" name="kode" type="text" class="mt-2" :value="old('kode')" required />
                    @error('kode')
                        <p class="mt-2 text-sm text-neutral-700 dark:text-neutral-300">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <x-input-label for="stocks" value="Stocks" />
                    <x-text-input id="stocks" name="stock" type="number" class="mt-2" :value="old('stock')" required />
                    @error('stock')
                        <p class="mt-2 text-sm text-neutral-700 dark:text-neutral-300">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <x-text-input id="beli" class="price-input" inputmode="numeric" name="beli" type="text" class="mt-2" :value="old('beli')" required />
                    @error('beli')
                        <p class="mt-2 text-sm text-neutral-700 dark:text-neutral-300">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <x-text-input id="jual" class="price-input" inputmode="numeric" name="jual" type="text" class="mt-2" :value="old('jual')" required />
                    @error('jual')
                        <p class="mt-2 text-sm text-neutral-700 dark:text-neutral-300">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="sm:col-span-2 flex flex-wrap gap-3">
                    <x-primary-button type="submit">Buat Stok</x-primary-button>
