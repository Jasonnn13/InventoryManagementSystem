<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="app-page-title">Ubah Pelanggan</h2>
            </div>
    ...
    <div class="mx-auto max-w-3xl">
        <section class="app-card">
            <form action="{{ route('customers.update', $customer->id) }}" method="POST" class="grid gap-5 sm:grid-cols-2">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" class="mt-2" value="{{ old('name', $customer->name) }}" required />
                </div>

                <div>
                    <x-input-label for="contact_information" value="Kontak" />
                    <x-text-input id="contact_information" name="contact_information" type="text" class="mt-2" value="{{ old('contact_information', $customer->contact_information) }}" />
                </div>

                <div class="sm:col-span-2">
                    <x-input-label for="address" value="Alamat" />
                    <x-text-input id="address" name="address" type="text" class="mt-2" value="{{ old('address', $customer->address) }}" required />
                </div>
