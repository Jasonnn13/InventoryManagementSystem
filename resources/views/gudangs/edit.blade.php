<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="app-page-title">Ubah Gudang</h2>
            </div>
    ...
    <div class="mx-auto max-w-2xl">
        <section class="app-card">
            <form action="{{ route('gudangs.update', $gudang->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <x-input-label for="name" value="Name" />
                    <x-text-input id="name" name="name" type="text" class="mt-2" value="{{ old('name', $gudang->name) }}" required autofocus />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>

                <div class="flex flex-wrap gap-3">
                    <x-primary-button type="submit">Perbarui Gudang</x-primary-button>
