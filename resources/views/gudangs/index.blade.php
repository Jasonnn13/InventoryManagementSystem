...
@endif
    ...
    <div class="container">
        ...
        <main class="main-content">
            ...
            <div class="search-form">
                <form action="{{ route('gudangs.index') }}" method="GET">
                ...
                <div class="category-header">
                    <h3>List Gudang</h3>
                    <a href="{{ route('gudangs.create') }}" class="btn btn-primary">Add gudang</a>
                </div>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr onclick="window.location='{{ route('stocks.index', ['search' => $gudang->name]) }}'">
                            <td data-label="Name">{{ $gudang->name }}</td>
                            <td data-label="Actions">
                                <div class="action-icons">
                                    <a href="{{ route('gudangs.edit', $gudang->id) }}" class="edit">
                                    <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('gudangs.destroy', $gudang->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
        ...
        <ul style="display: flex; list-style-type: none; padding: 0; margin: 0;">
            ...
            @else
                <li style="display: inline-block; margin: 0 15px;">
                    <a href="{{ $gudangs->previousPageUrl() }}" style="color: white; text-decoration: none; font-size: 18px; transition: color 0.3s ease;">&laquo;</a>
                </li>
            @endif

            {{-- Pagination Elements --}}
            @foreach ($gudangs->appends(['search' => $search])->links()->elements[0] as $page => $url)
                @if ($page == $gudangs->currentPage())
            ...
            {{-- Next Page Link --}}
            @if ($gudangs->hasMorePages())
                <li style="display: inline-block; margin: 0 15px;">
                    <a href="{{ $gudangs->nextPageUrl() }}" style="color: white; text-decoration: none; font-size: 18px; transition: color 0.3s ease;">&raquo;</a>
                </li>
            @else
    ...
    <script>
        ...
        function confirmation(event) {
            ...
            if (userLevel < 2) {
                swal({
                    title: "Sorry, you don't have permission to delete this gudang",
                ...
                swal({
                    title: "Are you sure you want to delete this gudang?",
                    text: "This action cannot be undone.",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
