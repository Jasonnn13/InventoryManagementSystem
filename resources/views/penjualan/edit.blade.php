<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="app-page-title">Edit Penjualan</h2>
            </div>
    ...
    <div class="mx-auto max-w-3xl">
        <section class="app-card">
            <form action="{{ route('penjualan.update', $penjualan->id) }}" method="POST" class="grid gap-5 sm:grid-cols-2">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="customers_id" name="customers_id" class="mt-2 block w-full rounded-xl border border-neutral-300 bg-white px-4 py-3 text-sm text-neutral-900 shadow-sm focus:border-black focus:ring-2 focus:ring-black dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100 dark:focus:border-white dark:focus:ring-white" required>
                        @foreach($customers as $customer)
                            <option value="{{ $customer->id }}" {{ old('customers_id', $penjualan->customers_id) == $customer->id ? 'selected' : '' }}>{{ $customer->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="tipe_ppn" name="tipe_ppn" class="mt-2 block w-full rounded-xl border border-neutral-300 bg-white px-4 py-3 text-sm text-neutral-900 shadow-sm focus:border-black focus:ring-2 focus:ring-black dark:border-neutral-700 dark:bg-neutral-950 dark:text-neutral-100 dark:focus:border-white dark:focus:ring-white" required>
                        <option value="PPN" {{ old('tipe_ppn', $penjualan->tipe_ppn) == 'PPN' ? 'selected' : '' }}>PPN</option>
                        <option value="Non PPN" {{ old('tipe_ppn', $penjualan->tipe_ppn) == 'Non PPN' ? 'selected' : '' }}>Non PPN</option>
                    </select>
                </div>

                <div>
                    <x-input-label for="sales" value="Sales" />
                    <x-text-input id="sales" name="sales" type="text" class="mt-2" value="{{ old('sales', $penjualan->sales) }}" required />
                </div>

                <div>
                    <x-input-label for="tenggat_waktu" value="Tenggat" />
                    <x-text-input id="tenggat_waktu" name="tenggat_waktu" type="date" class="mt-2" value="{{ \Carbon\Carbon::parse(old('tenggat_waktu', $penjualan->tenggat_waktu))->format('Y-m-d') }}" required />
                </div>

                <div>
                    <x-input-label for="diskon" value="Diskon (%)" />
                    <x-text-input id="diskon" name="diskon" type="number" min="0" max="100" class="mt-2" value="{{ old('diskon', $penjualan->diskon) }}" required />
                </div>
